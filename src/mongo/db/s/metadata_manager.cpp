...
//
//  CollectionMetadata objects pointed to from _metadata are maintained at least as long as any
//  query holds a ScopedCollectionMetadata object referring to them, or to any older tracker. In the
//  diagram above, the middle CollectionMetadata must be kept until the one below it is disposed of.
...
void MetadataManager::_retireExpiredMetadata() {
    ...
    for (; _metadata.front()->_tracker.usageCounter == 0; _metadata.pop_front()) {
        ...
        if (_metadata.begin() == --_metadata.end())
...
bool MetadataManager::_overlapsInUseChunk(ChunkRange const& range) {
    ...
    for (auto it = _metadata.begin(), end = _metadata.end(); it != end; ++it) {
        // refcount doesn't matter for the active metadata:
        if (((*it)->_tracker.usageCounter != 0 || it == --_metadata.end()) &&
            (*it)->rangeOverlapsChunk(range)) {
            return true;
        }
    }
    return false;
