...
bool CollectionRangeDeleter::cleanUpNextRange(OperationContext* opCtx,
    ...
    WriteConcernResult wcResult;  // ignore it.
    Status status = Status::OK();
    try {
        status = waitForWriteConcern(opCtx, clientOpTime, kMajorityWriteConcern, &wcResult);
    } catch (DBException& e) {
        status = e.toStatus();
    }
    if (!status.isOK()) {
